...
استقرار Docker شامل image مجزای هر نقش، migration gate، healthcheck، محدودیت
منابع، دقیقاً یک replica برای هر worker، لاگ JSON چرخشی و Ollama مستقل است. build
و اجرای عادی هیچ مدلی را دانلود نمی‌کنند؛ pull مدل 3B محلی یا 14B سرور فقط با
profile دستی انجام می‌شود. دستورهای کامل در
[`docs/deployment.md`](docs/deployment.md) قرار دارند.
...
پس از پایان هر job استخراج، کمترین قیمت هر ایرلاین برای همان وب‌سایت و درخواست در دیتابیس ذخیره می‌شود. ذخیره به‌صورت جایگزینی atomic نتیجهٔ قبلی provider است؛ بنابراین افزایش قیمت یا حذف یک پرواز نیز نتیجهٔ قدیمی را باقی نمی‌گذارد و اگر worker قدیمی دیرتر تمام شود نمی‌تواند snapshot جدیدتر را بازنویسی کند.

صفحهٔ `/control/search-box/page` داده‌های ذخیره‌شده را بین وب‌سایت‌ها بر اساس نام رسمی ایرلاین تجمیع می‌کند و کمترین قیمت هر ایرلاین را نشان می‌دهد. تاریخ آخرین به‌روزرسانی نتایج بالای صفحه دیده می‌شود. اجرای مراحل ناهمزمان است و صفحه منتظر workerها نمی‌ماند؛ اگر یک وب‌سایت شکست بخورد، آخرین نتیجهٔ سالم و نتیجهٔ وب‌سایت‌های دیگر باقی می‌ماند. نتیجهٔ JSON همان درخواست از مسیر زیر قابل دریافت است:

```text
GET /control/search-box/{search_request_id}/results
```

برای ساخت جدول‌های نتیجه، پس از دریافت این نسخه `alembic upgrade head` را اجرا کنید. خروجی‌های قدیمی از اولین اجرای بعدی extractor برای هر سرچ وارد این جدول‌ها می‌شوند.

اعداد قیمت با همان واحد نمایش‌داده‌شده در منبع استخراج می‌شوند؛ پیش از اضافه‌کردن providerهایی با واحد متفاوت باید نرمال‌سازی واحد به extractor افزوده شود.
